...
class _NestImage:
    class _CRUD(Resource):
        ...
        def post(self):
            ...
            # Checks if the id for the image exists
            if 'nestImg' not in request.json:
                return {'message': 'Base64 image data required.'}, 400
            base64_image = request.json['nestImg']
            
            return nestImg_base64_upload(base64_image, data['file_name'], current_user.uid)
    ...
    api.add_resource(_CRUD, '/nestImg')
    api.add_resource(_FETCH, '/nestImgFetch')
